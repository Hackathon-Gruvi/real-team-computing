...
def get_imdb_id_duckduckgo_scraping(movie_title):
    ...
    query = "https://www.google.pt/search?q=" + movie_title + "+site%3Aimdb.com%2Ftitle"
    html_text = requests.get(query, headers={"accept-language": "en-US", "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.114 Safari/537.36"}).text
    soup = BeautifulSoup(html_text, 'html.parser')
...
def get_imdb_id_google_api(movie_title):
    ...
    url = "https://www.googleapis.com/customsearch/v1/siterestrict?key=" + key + "&cx=" + cx + "&q=" + movie_title + "&num=1"
...
def get_imdb_id_google_scraping(movie_title):
    movie_title = urllib.parse.quote_plus(movie_title)
    query = "https://www.google.com/search?q=" + movie_title + "+site%3Aimdb.com%2Ftitle"
    html_text = requests.get(query, headers={"accept-language": "en-US", "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.114 Safari/537.36"}).text
    soup = BeautifulSoup(html_text, 'html.parser')
    top_result = soup.body.find('div', {"class": "g"})
    return top_result
        ...
        if repeat:
            ...
            for i in range(max_cut):
                ...
                if id_first is not None:
                    fscore = str_compare_word_match(get_title(id_first), t_first)
                    #print(fscore)
            ...
            if len(imdb_id_list) == 0:
                ...
                for word in title_list:
                    ...
                    if temp_id is not None:
                        ...
                        temp_score = simple_word_compare(get_title(temp_id), word)
...
with open(in_filename, newline='') as in_file:
    with open(out_filename, 'w', newline='') as out_file:
        ...
        for title in reader:
            ...
            writer.writerow([get_imdb_id_google_api(title[0]), title[0]])
            num_processed += 1
        end_time = time.time()
        elapsed_time = end_time - start_time
        avg_time = elapsed_time/num_processed
        print("Processed " + str(num_processed) + "titles in " + str(elapsed_time) + "s" + "(average " + str(avg_time) + "s/title)")
